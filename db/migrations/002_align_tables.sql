-- Fazle Core — Migration 002: Table Alignment
-- Safe to re-run (IF NOT EXISTS / IF NOT EXISTS column guards throughout).
-- Applies: fazle_knowledge_base, fazle_recruitment_sessions, fazle_payment_drafts
--          + missing columns on fazle_draft_replies
--          + all indexes
--          + 14 KB seed entries (ON CONFLICT DO UPDATE — idempotent)
...
CREATE INDEX IF NOT EXISTS idx_kb_key      ON fazle_knowledge_base(key);
...
CREATE TABLE IF NOT EXISTS fazle_recruitment_sessions (
    ...
    collection_step     TEXT        DEFAULT 'name',
    funnel_stage        TEXT        DEFAULT 'collecting',
    score               INTEGER,
    score_bucket        TEXT,
...
CREATE INDEX IF NOT EXISTS idx_recruit_bucket ON fazle_recruitment_sessions(score_bucket);
...
CREATE INDEX IF NOT EXISTS idx_pdraft_ts     ON fazle_payment_drafts(created_at DESC);

-- ── Align fazle_draft_replies (columns added in 001 but guarded here for safety) ─
ALTER TABLE fazle_draft_replies
    ADD COLUMN IF NOT EXISTS status      TEXT        DEFAULT 'pending',
    ADD COLUMN IF NOT EXISTS approved_at TIMESTAMPTZ,
    ADD COLUMN IF NOT EXISTS admin_phone TEXT;

CREATE INDEX IF NOT EXISTS idx_draft_status ON fazle_draft_replies(status);

-- ── Knowledge Base Seed Data — 14 entries ──────────────────────────────────────
INSERT INTO fazle_knowledge_base (category, key, trigger_keywords, reply_text, reply_short) VALUES

('faq', 'greeting_response',
 ARRAY['সালাম','আস্সালামু','hello','hi','হ্যালো','menu','মেনু','start'],
...
('recruitment', 'negative_comment',
 ...
 'আমরা একটি নিবন্ধিত প্রতিষ্ঠান। কোনো ফি বা জামানত নেওয়া হয় না। সরাসরি অফিসে এসে যাচাই করুন।'),
...
('recruitment', 'fee_suspicion',
 ARRAY['টাকা লাগবে','ভর্তি ফি','জামানত','joining fee','deposit','registration fee','uniform fee'],
